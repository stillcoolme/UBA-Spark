...
用户访问行为分析项目
## 1. 本项目功能
用户在网站内的访问过程，就称之为一次session。
简单理解，session就是某一天某一个时间段内，某个用户对网站从打开/进入，到做了大量操作，到最后关闭浏览器的过程。就叫做session。
1. 对用户访问session进行分析，筛选出指定的一些用户（有特定年龄、职业、城市）
2. 用户在指定日期范围内访问session聚合统计，比如，统计出访问时长在0~3s的session占总session数量的比例
3. 按时间比例随机抽取session。比如一天有24个小时，其中12:00~13:00的session数量占当天总session数量的50%，当天总session数量是10000个，那么当天总共要抽取1000个session，12:00~13:00的用户，就得抽取1000*50%=500。而且这500个需要随机抽取。
...
2. flatMapTopair不知道什么鬼。返回的tuple2的key，value是同一个值？？？？
3. join也不会了。


### 2.3 top10热门品类
需求回顾：
通过筛选条件的session访问过的所有品类（点击、下单、支付），按照各个品类的点击、下单和支付次数，降序排序。
获取前10个品类，top10热门品类；
